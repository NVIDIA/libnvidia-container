...
#define cuIpcOpenMemHandle                  cuIpcOpenMemHandle_v2
    ...
    #define cuStreamSetFlags                    __CUDA_API_PTSZ(cuStreamSetFlags)
...
typedef enum CUstream_flags_enum {
    ...
    CU_STREAM_SCHED_AUTO          = 0x0, /**< Use context flags for synchronization */
    CU_STREAM_SCHED_SPIN          = 0x2, /**< Set spin as default scheduling */
    CU_STREAM_SCHED_YIELD         = 0x4, /**< Set yield as default scheduling */
    CU_STREAM_SCHED_BLOCKING_SYNC = 0x8, /**< Set blocking synchronization as default scheduling */
    CU_STREAM_SCHED_MASK          = 0xe,
    CU_STREAM_FLAGS_MASK          = 0xf
} CUstream_flags;
...
typedef enum CUdevice_attribute_enum {
    ...
    CU_DEVICE_ATTRIBUTE_MAX_PERSISTING_L2_CACHE_SIZE = 108,                 /**< Device's maximum L2 persisting lines capacity setting > */
 ...
 * determines behaviors of the stream. It can also be used to control how the OS
 * thread, which owns the CUDA stream at the time of an API call, interacts
 * with the OS scheduler when waiting for results from the GPU. Only one of
 * the scheduling flags can be set. If conflicting flags are passed,
 * ::CUDA_ERROR_INVALID_VALUE is returned.
 *
 ...
 * - ::CU_STREAM_SCHED_AUTO: Instructs CUDA to use context policy for synchronization,
 *   similar to passing the \p flags parameter as 0.
 * - ::CU_STREAM_SCHED_SPIN: Instruct CUDA to actively spin when waiting for
 *   results from the GPU. This can decrease latency when waiting for the GPU,
 *   but may lower the performance of CPU threads if they are performing work in
 *   parallel with the CUDA thread.
 * - ::CU_STREAM_SCHED_YIELD: Instruct CUDA to yield its thread when waiting for
 *   results from the GPU. This can increase latency when waiting for the GPU,
 *   but can increase the performance of CPU threads performing work in parallel
 *   with the GPU.
 * - ::CU_STREAM_SCHED_BLOCKING_SYNC: Instruct CUDA to block the CPU thread on a
 *   synchronization primitive when waiting for the GPU to finish work.
 *
...
/**
 * \brief Set flags for the stream
 *
 * Sets the flags for the stream overwriting previously set ones.
 * ::cuStreamSetFlags overwrites all flags, so if any flag is not set 
 * in \p flags, it will be unset. It is important to note that flags that change
 * semantics of stream, such as ::CU_STREAM_NON_BLOCKING, cannot be changed and
 * must be set if they have been set during creation. For that reason user should
 * query using ::cuStreamGetFlags and overwrite desired ones if they don't control the creation flags.
 *
 * The \p flags parameter can be used to control how the OS
 * thread, which owns the CUDA stream at the time of an API call, interacts
 * with the OS scheduler when waiting for results from the GPU. Only one of
 * the scheduling flags can be set. If conflicting flags are passed,
 * ::CUDA_ERROR_INVALID_VALUE is returned.
 ...
 * ::cudaStreamSetFlags
 */
CUresult CUDAAPI cuStreamSetFlags(CUstream hStream, unsigned int flags);
